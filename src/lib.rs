...
lazy_static! {
  ...
  static ref RNG: Mutex<Rand32> = Mutex::new(Rand32::new(DEFAULT_SEED.into()));
}
      ...
      match (rows_value.as_u64(), columns_value.as_u64()) {
        (Some(rows_u64),Some(columns_u64)) => {
          ...
          for out_ix in out.linear_index_iterator() {
            ...
            out.set_unchecked_linear(out_ix, Value::from_f32(random_value));
          }
        }
        _ => (),
      }
    ...
    for ((rows_value,_),(columns_value,_)) in rows.clone().zip(columns.clone()) {
      match (rows_value.as_u64(), columns_value.as_u64()) {
        (Some(rows_u64),Some(columns_u64)) => {
          ...
          let mut rng = RNG.lock().unwrap();
          for out_ix in out.linear_index_iterator() {
            let mut random_value = rng.rand_u32();
            out.set_unchecked_linear(out_ix, Value::from_u32(random_value));
          }
        }
        _ => (),
      }
